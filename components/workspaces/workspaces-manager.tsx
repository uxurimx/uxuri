...
import {
  ...
  X, Check, ChevronRight, Building2, User, Loader2, Globe,
} from "lucide-react";
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {!isActive ? (
              <button
                ...
                onClick={activate}
                disabled={activating}
                className="flex items-center gap-1.5 px-3 h-8 rounded-lg text-xs font-semibold bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 transition"
              >
                {activating
...
function GlobalModeCard({ isActive, onActivated }: { isActive: boolean; onActivated: () => void }) {
  const [activating, setActivating] = useState(false);
  ...
  async function activate() {
    ...
    try {
      await fetch("/api/workspaces/active", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ workspaceId: "global" }),
      });
      onActivated();
    } finally {
      setActivating(false);
  ...
  return (
    <article className={cn(
      ...
      isActive
        ? "border-indigo-400 dark:border-indigo-500 ring-2 ring-indigo-400/20 bg-indigo-50/50 dark:bg-indigo-950/20"
        : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-slate-300 dark:hover:border-slate-600"
    )}>
      <div className="h-1.5 bg-indigo-500" />
      <div className="flex items-center gap-4 p-5">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 bg-indigo-100 dark:bg-indigo-900/40">
          <Globe className="w-7 h-7 text-indigo-500" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="font-bold text-slate-900 dark:text-slate-100 text-lg leading-tight">Vista global</h2>
            {isActive && (
              <span className="inline-flex items-center px-2 py-0.5 text-[10px] font-bold rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400">
                Activo
              </span>
            )}
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Ver todos los datos de todos los workspaces sin filtro
          </p>
        </div>
        {!isActive && (
          <button
            ...
            onClick={activate}
            disabled={activating}
            className="flex items-center gap-1.5 px-3 h-8 rounded-lg text-xs font-semibold bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50 transition flex-shrink-0"
          >
            {activating
              ? <Loader2 className="w-3 h-3 animate-spin" />
              : <Check className="w-3 h-3" />}
...
export function WorkspacesManager({
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Cada workspace tiene sus propios datos y perfiles de acceso. Activa uno desde aquí o desde el selector en el topbar.
          </p>
        </div>
        <button
          type="button"
      ...
      <GlobalModeCard isActive={isGlobalMode} onActivated={() => router.refresh()} />
      ...
      <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 p-5">
        ...
        <ul className="text-sm text-slate-500 dark:text-slate-400 space-y-1 mt-2">
          ...
          <li>• Cambia de workspace activo con el botón "Activar" en cada tarjeta, o desde el selector en el topbar</li>
