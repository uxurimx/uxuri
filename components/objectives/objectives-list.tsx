...
const horizonConfig: Record<HorizonKey, { label: string; className: string; emoji: string }> = {
  daily:     { label: "Diario",      className: "bg-green-50 text-green-700",   emoji: "📅" },
  weekly:    { label: "Semanal",     className: "bg-blue-50 text-blue-700",     emoji: "📆" },
  monthly:   { label: "Mensual",     className: "bg-purple-50 text-purple-700", emoji: "🗓" },
  quarterly: { label: "Trimestral",  className: "bg-amber-50 text-amber-700",   emoji: "🎯" },
  yearly:    { label: "Anual",       className: "bg-orange-50 text-orange-700", emoji: "⭐" },
  life:      { label: "Vida",        className: "bg-rose-50 text-rose-700",     emoji: "🌟" },
};
...
export function ObjectivesList({ initialObjectives }: ObjectivesListProps) {
  ...
  const filteredObjectives = objectives.filter(
    (obj) =>
      ...
      (horizonFilter === "all" || obj.horizon === horizonFilter)
  );
          ...
          <div className="flex gap-2 overflow-x-auto">
            <button
              ...
              className={cn(
                ...
                statusFilter === "all" ? "bg-[#1e3a5f] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              )}
            >
              Todos
            </button>
            {(["active", "draft", "paused", "completed", "cancelled"] as const).map((s) => (
              <button
                ...
                className={cn(
                  "whitespace-nowrap px-3 py-1.5 text-sm font-medium rounded-full transition-colors",
                  statusFilter === s ? "bg-[#1e3a5f] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                )}
              >
          ...
          <div className="flex gap-2 overflow-x-auto">
            <button
              ...
              className={cn(
                ...
                horizonFilter === "all" ? "bg-slate-700 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"
              )}
            >
              Todos los horizontes
            </button>
            {(Object.keys(horizonConfig) as HorizonKey[]).map((h) => (
              <button
                key={h}
                onClick={() => setHorizonFilter(h)}
                className={cn(
                  "whitespace-nowrap px-3 py-1 text-xs font-medium rounded-full transition-colors",
                  horizonFilter === h ? horizonConfig[h].className + " ring-1 ring-current" : "bg-slate-100 text-slate-500 hover:bg-slate-200"
                )}
              >
                {horizonConfig[h].emoji} {horizonConfig[h].label}
              </button>
            ))}
          </div>
        </div>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    {obj.horizon && horizonConfig[obj.horizon as HorizonKey] && (
                      <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium", horizonConfig[obj.horizon as HorizonKey].className)}>
                        {horizonConfig[obj.horizon as HorizonKey].emoji} {horizonConfig[obj.horizon as HorizonKey].label}
                      </span>
                    )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleCreate} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Horizonte</label>
                <select
                  value={horizon}
                  onChange={(e) => setHorizon(e.target.value as HorizonKey | "")}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/20"
                >
                  <option value="">Sin horizonte</option>
                  {(Object.keys(horizonConfig) as HorizonKey[]).map((h) => (
